...
class CSVParser:
    ...
    def _cast_val(value: str, field_name: str) -> str | int | float:
        ...
        if field_name == "id":
            ...
            try:
                ...
                raise ValueError(
                    f"Передано неверное значение для поля {field_name}: {value}"
                )
        else:
            return value
    ...
    def parse(self) -> list[dict[str, str | int | float]]:
        ...
        if not required_columns.issubset(self.headers):
            ...
            raise ValueError(
                f"В файле {self.file_path} не хватает обязательных колонок в header: {missing}"
            )
        ...
        for line in lines[1:]:
            ...
            if len(split_values) != len(self.headers):
                raise ValueError(
                    f"Строка содержит {len(split_values)} колонок, ожидалось {len(self.headers)}"
                )
            values = [
                self._cast_val(val, field)
                for val, field in zip(split_values, self.headers)
            ]
